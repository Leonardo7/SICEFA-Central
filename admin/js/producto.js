...
export function saveProducto()
{
    let pro = null;
    let posicion = -1; //Para saber si un producto ya existe o aun no
    let idProductos = 0;

    //Revisamos si hay un ID de producto:
    if (document.getElementById("txtIdProductos").value.trim().length > 0)
    {
        idProductos = parseInt(document.getElementById("txtIdProductos").value.trim());
        posicion = buscarPosicionProductoPorId(idProductos);

        //Si posicion es mayor o igual a 0, si encontramos un producto:
        if (posicion >= 0)
            pro = producto[posicion];
        else
        {
            //Si no hay un producto con el ID escrito,
            //creamos una nueva instancia del Objeto:
            pro = new Object();
            pro.id = idProductos;

            //Insertamos el objeto pro dentro del arreglo de producto:
            producto.push(pro);
        }

        //Continuamos llenando los datos del objeto:
        pro.id = document.getElementById("txtIdProductos").value;
        pro.nombre = document.getElementById("txtNombre").value;
        pro.nombreGenerico = document.getElementById("txtNomGen").value;
        pro.formaFarmaceutica = document.getElementById("txtForFar").value;
        pro.unidadMedida = document.getElementById("txtUniMe").value;
        pro.presentacion = document.getElementById("txtPresentacion").value;
        pro.principalIndicacion = document.getElementById("txtPrincIn").value;
        ...
        pro.unidadesEnvase = document.getElementById("txtUnidadEnv").value;
        pro.precioCompra = document.getElementById("numPrecio").value;
        pro.precioVenta = document.getElementById("numPrecioVent").value;
        pro.foto = document.getElementById("foto").value;
        pro.rutaFoto = document.getElementById("txtRuta").value;
        pro.codigoBarras = document.getElementById("txtCodBarras").value;
        pro.estatus = document.getElementById("numEstatus").value;

        //Refrescamos el catalogo de producto:
        fillTableProducto();

        Swal.fire('Movimiento Realizado', 'Datos de Producto Actualizados correctamente', 'success');
    } else
    {
        Swal.fire('Verifica&oacute;n de datos requerida',
                'Debe agregar un ID al producto (valor num&eacute;rico)',
                'warning');
...
export function setDetalleProductoVisible(valor)
{
    ...
    {
        //Ocultamos la sección del detalle de productos
